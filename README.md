...
`http://localhost:3000` 접속 → `.env`의 `APP_PASSWORD`로 로그인 → 식당 선택.
...
- **업체/계약 관리**: 업체 등록, 계약(계약기간+단가표) 등록. 같은 업체·식당에 기간이 겹치는 계약은 생성이 차단됩니다.
- **거래명세표 입력**: 수기입력 또는(OCR 설정 시) PDF/JPG 업로드 자동 인식. 품목명 입력 시 활성 계약의 단가가 자동으로 채워지며, 정확히 일치하지 않으면 유사 품목을 제안합니다.
